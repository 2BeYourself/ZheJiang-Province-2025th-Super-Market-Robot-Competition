...
//static uint8_t  echo_state[US_CHANNEL_NUM];    // 0: 等待上升沿, 1: 等待下降沿 可以理解为echo的上一个状态,lvl为现在状态
...
//    for (int i = 0; i < US_CHANNEL_NUM; i++)
//    {
//        char sem_name_s[RT_NAME_MAX];   //这是啥？->给信号量动态命名
//        char sem_name_e[RT_NAME_MAX];
//
//        rt_sprintf(sem_name_s, "us_s%d", i);    //动态命名
