...
rt_err_t gyro_uart_init(const char *uart_name)
{
    ...
    rt_kprintf("%s opened (INT RX mode)\n", uart_name);
    return RT_EOK;
}
...
/* 用于调试：统计接收回调被调用的次数 */
volatile uint32_t uart2_rx_callback_count = 0;
volatile uint32_t uart2_rx_bytes = 0;

/*
 * UART2 接收回调函数（在中断上下文中调用）
 * 参考官方例程的 USART2_IRQHandler
 */
static rt_err_t uart2_rx_ind(rt_device_t dev, rt_size_t size)
{
    uint8_t ch;
    uart2_rx_callback_count++;  // 统计回调次数

    while (rt_device_read(dev, 0, &ch, 1) == 1)
    {
        uart2_rx_bytes++;  // 统计接收字节数
        WitSerialDataIn(ch);  // 每收到一个字节就送入SDK解析
    }
    return RT_EOK;
}

/* 读取陀螺仪的Yaw值 - 只用于启动时的初始化 */
void uart2_recv_thread(void *parameter)
{
    RT_ASSERT(gyro_uart);

    /* 设置接收回调 */
    rt_device_set_rx_indicate(gyro_uart, uart2_rx_ind);

    rt_kprintf("UART2 RX callback registered\n");
...
static void AutoScanSensor(void)//串口波特率检测
{
    ...
    {
        /* 动态改变 UART2 波特率（参考官方例程的 Usart2Init()） */
        config.baud_rate = c_uiBaud[i];
        rt_device_control(gyro_uart, RT_DEVICE_CTRL_CONFIG, &config);

        rt_kprintf("Trying %d baud...\r\n", c_uiBaud[i]);
