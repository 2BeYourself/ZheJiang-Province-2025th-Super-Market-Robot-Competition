...
/* 升降电机RPM设置 */
volatile uint16_t g_elevator_rpm = 100;  /* 默认100 RPM */

/* 运动状态 */
static motor_state_t cur_state = ST_IDLE;

/* 升降电机命令发送计数器（发送3次确保可靠性） */
static uint8_t elevator_cmd_count = 0;
#define ELEVATOR_CMD_RETRY_COUNT 3  /* 升降电机命令重复发送次数 */
  ...
  if (cur_state != st)  // 只有状态真正切换时才重置
  {
      switch (st)
      {
          ...
          case ST_Elevator_Stop:
              elevator_cmd_count = 0;  // 重置计数器，允许新状态发送命令
              break;
          default:
              // 其他状态不处理 elevator_cmd_count
              break;
      }
  }
...
/* 运动状态切换
 * - 42电机（轮子）：需要持续发送命令进行PID控制
 * - 57电机（升降台）：只需发送一次命令即可持续运行
 */
void motor_state_poll(void)
{
  ...
  /* 升降电机：只发送固定次数的命令（3次确保可靠性），然后不再发送 */
  case ST_UP:
      if (elevator_cmd_count < ELEVATOR_CMD_RETRY_COUNT)
      {
...
{
//    motor_cmd_speed(0x05, CW,  rpm, 0, RT_NULL);
...
{
//    motor_cmd_speed(0x05, CCW,  rpm, 0, RT_NULL);
  motor_cmd_position(Elevator,
                      CCW,
                      rpm,
                      0,
